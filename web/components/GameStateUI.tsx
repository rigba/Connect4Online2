...
const GameStateUI = ({
  ...
  const [rematch, rematchInfo] = useMutation(RematchDocument);
  ...
  if (game?.error?.message.search("does not exist")) {
    router.replace("/");
  }
  ...
  if (
    game?.data?.fetchGame?.id &&
    user.data?.me?.id &&
    game?.data?.fetchGame?.joinedID
  ) {
    if (game?.data.fetchGame?.winner) {
      return (
        <div className="flex flex-col align-middle items-center text-center w-full px-4">
          <h1 className="font-public-md text-xl my-1">🏆</h1>
          <h1 className="font-public-lg text-xl my-1">{`${
            game.data.fetchGame.winner === game.data.fetchGame.createdId
              ? game.data.fetchGame.created.username
              : game.data.fetchGame.joined.username
          }`}</h1>

          <h1 className="font-public-md text-md">is the winner!</h1>
          <button
            className={`${
             ( game.data.fetchGame.rematch.includes(game.data.fetchGame.joinedID) || game.data.fetchGame.rematch.includes(game.data.fetchGame.createdId)) &&
              "hover-ani"
            } text-white bg-blue-700 focus:outline-none focus:ring-2 font-public-rg rounded-lg text-sm w-32 p-2 my-1 text-center mx-auto`}
            onClick={() => {
              rematch({ variables: { gameId } });
            }}
          >
            {game.data.fetchGame.rematch.includes(user.data?.me?.id) || rematchInfo.loading ? (
              <div role="status">
                <svg
                  aria-hidden="true"
                  className="w-6 h-6 text-blue-700 animate-spin fill-white mx-auto"
                  viewBox="0 0 100 101"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
    ...
    "
      ...
      // your move
      return <h1 className="font-public-lg text-xl my-1">opponents turn</h1>;
    }
  } else {
    return (
      <form
